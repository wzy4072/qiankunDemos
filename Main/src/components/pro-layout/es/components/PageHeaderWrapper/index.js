...
var renderFooter = function renderFooter (h, tabConfigProps, i18nRender) {
  ...
  return tabList && tabList.length > 0 && h(Tabs, _mergeJSXProps([{
...
var defaultPageHeaderRender = function defaultPageHeaderRender (h, props, pageMeta, i18nRender) {
  var propTitle = props.title
      ...
      var restProps = _objectWithoutProperties(props, ['title', 'tags', 'content', 'pageHeaderRender', 'extra', 'extraContent', 'breadcrumb', 'back'])
  ...
  return h(PageHeader, _mergeJSXProps([{
    props: tabProps
  }, {
    on: {
...
var PageHeaderWrapper = {
  ...
  render: function render (h) {
    ...
    if (propsBreadcrumb === true) {
        var routes = []
        var routes1 = $route.matched.concat()
        for (var i = 0; i < routes1.length; i++) {
            var route1 = routes1[i]
            if (route1.meta.parentPath) {
                routes.push({
                    path: route1.meta.parentPath,
                    breadcrumbName: i18n(route1.meta.parentTitle)
                })
            }
            routes.push({
                path: route1.path,
                breadcrumbName: i18n(route1.meta.title)
            })
        }

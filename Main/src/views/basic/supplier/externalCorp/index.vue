    ...
    <a-card :bordered="false" style="margin-top: 24px">
      ...
      >
        <template slot="action" slot-scope="text, row">
          <a v-if="row.auditStatus !== 11" @click="editItem(row.id)">{{ $t('common.btn.edit') }}</a>
          ...
          <a-divider v-if="row.auditStatus !== 11" type="vertical" />
          <a v-if="row.auditStatus !== 11" @click="deleteRow(row.id)">{{ $t('common.btn.delete') }}</a>
        </template>
        <template slot="stitle">
          ...
          <a-dropdown :disabled="selectedRowKeys.length === 0">
            <a-menu slot="overlay">
              ...
              <a-menu-item @click="batchSubmit">{{ $t('common.btn.batch.submit') }}</a-menu-item>
            </a-menu>
            <a-button>
...
import { deleteExternalCorp, searchExternalCorp, batchSubmit } from '@/api/basic/supplier/externalCorp'
    ...
    data () {
        return {
            ...
            loadData: parameter => {
                ...
                return searchExternalCorp(requestParameters).then(res => res.data)
            },
            selectedRowKeys: [],
    ...
    methods: {
        addItem () {
            ...
            this.$store.commit('SET_ASYNC_TABS', {
                componentDirPath: '/basic/supplier/externalCorp/edit', // 必填 组件地址 '@/views/supplier/externalCorp/edit.vue'
                props: { id: null }, // 非必填 组件需要的参数
                title: this.$t('basic.externalCorp.title.add'), // 必填
        ...
        detailItem (id) {
            const pageName = 'externalCorpDetail'
            const pageSign = id
            const tabFullPath = `/commonEdit/${pageName}/id/${pageSign}`
            this.$store.commit('SET_ASYNC_TABS', {
                componentDirPath: '/basic/supplier/externalCorp/descriptionsModal', // 必填 组件地址 '@/views/supplier/externalCorp/edit.vue'
                props: { id }, // 非必填 组件需要的参数
                title: this.$t('basic.externalCorp.title.view'), // 必填
                parentPath: this.$route.fullPath, // 必填 组件tab关闭后 返回的页面
                tabFullPath // 必填 子组件的fullPath路径
            })
            this.$router.push({ path: tabFullPath })
        ...
        editItem (id) {
            ...
            this.$store.commit('SET_ASYNC_TABS', {
                componentDirPath: '/basic/supplier/externalCorp/edit', // 必填 组件地址 '@/views/supplier/externalCorp/edit.vue'
                props: { id }, // 非必填 组件需要的参数
                title: this.$t('basic.externalCorp.title.edit'), // 必填
                parentPath: this.$route.fullPath, // 必填 组件tab关闭后 返回的页面
                tabFullPath // 必填 子组件的fullPath路径
            })
            this.$router.push({ path: tabFullPath })
        ...
        deleteRow (id) {
            ...
            this.$confirm({
                ...
                onOk () {
                    return deleteExternalCorp([id]).then(response => {
                        if (response.success) {
                            // 操作成功
        ...
        deleteData () {
            ...
            this.$confirm({
                ...
                content: this.$t('common.confirm.delete'),
                onOk () {
                    return deleteExternalCorp(keys).then(response => {
                        if (response.success) {
                            // 操作成功
                            _this.$message.success(_this.$t(response.msg))
                            _this.$refs.table.refresh(true)
        ...
        batchSubmit () {
            ...
            this.$confirm({
                title: this.$t('common.title.info'),
                content: this.$t('common.btn.submit'),
                onOk () {
                    return batchSubmit(keys).then(response => {
                        if (response.success) {
                            // 操作成功
                            _this.$message.success(_this.$t(response.msg))
                            _this.$refs.table.refresh(true)

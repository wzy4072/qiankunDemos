...
import { loadModuleMessage } from '@/locales/asyncMergeMessages'
  ...
  actions: {
    setLang ({ commit }, lang) {
      return new Promise((resolve, reject) => {
        ...
        const moduleName = location.pathname.substring(1).replace(/(\/)/g, '.')
        loadLanguageAsync(lang).then(() => {
          // 切换语言成功后 加载当前语言/当前模块的语言补充包、
          loadModuleMessage(moduleName)
          resolve()
        }).catch((e) => {
          reject(e)
        })
